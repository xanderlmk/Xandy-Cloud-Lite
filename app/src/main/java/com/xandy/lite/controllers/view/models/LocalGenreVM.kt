...
class LocalGenreVM(
    ...
    val genre = songRepository.pickedLocalGenre.stateIn(
    ...
    fun selectSong(song: AudioFile, list: List<AudioFile>, genreName: String) {
        mediaController.value?.let { ctrl ->
            Controller.setQueue(ctrl, list, song, appStrings.value) {
                viewModelScope.launch { songRepository.setNewQueue(it, genreName) }
            }
